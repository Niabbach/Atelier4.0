                ...
                for(int j = 0; j < currentSimulation.demandedProducts.size(); j++){
                    ...
                    for(int i = 0; i < product.amount; i++){
                        ...
                        ic.products.get(product.name).stages.get(lastStageId).forEach(action ->{
                            ...
                            ic.Robots.values().forEach(Robot -> {
                                Robot.actions.forEach(RobotAction -> {
                                    if(RobotAction.productName.equals(product.name)
                                            && RobotAction.stageId == lastStageId
                                            && RobotAction.actionName.equals(action.actionName)){
                                        ACLMessage msg = new ACLMessage();
                                        AID receiver = new AID("Robot" + Integer.toString(Robot.RobotId), AID.ISLOCALNAME);
    ...
    SimpleBehaviour ChooseOffer(Integer productId){
        return new SimpleBehaviour() {
            ...
            public void action() {
                ...
                int bestIdx = 0;
                for (int i = 0; i < PlanMap.get(productId).size(); i++){
                    if(PlanMap.get(productId).get(i)._productionEnd == -1) //when Robot is broke
                        continue;
